...
#define MM_CLASS(CLASS, ...)                                       \
  using _yomm11_base_list = ::yorel::methods::yomm11_class::base_list<__VA_ARGS__>; \
  YOMM11_TRACE(friend const char* _yomm11_name_(CLASS*) { return #CLASS; })     \
  virtual void _yomm11_init_class_() { &::yorel::methods::yomm11_class::initializer<CLASS, ::yorel::methods::yomm11_class::base_list<__VA_ARGS__>>::the; }

#undef MM_EXTERN_CLASS
#define MM_EXTERN_CLASS(CLASS)

#define MM_FOREIGN_CLASS(CLASS, ...)                               \
  static_assert(::yorel::methods::detail::check_bases<CLASS, ::yorel::methods::yomm11_class::base_list<__VA_ARGS__>>::value, "error in MM_FOREIGN_CLASS(): not a base in base list"); \
  ...
  namespace { ::yorel::methods::yomm11_class::initializer<CLASS, ::yorel::methods::yomm11_class::base_list<__VA_ARGS__>> _yomm11_add_class_ ## CLASS; }
...
#define MULTI_METHOD(ID, RETURN_TYPE, ...)                          \
  ...
  YOMM11_TRACE(inline const char* _yomm11_name_(::yorel::methods::method<ID ## _specialization, RETURN_TYPE(__VA_ARGS__)>*) { return #ID; })     \
  YOMM_CONSTEXPR ::yorel::methods::method<ID ## _specialization, RETURN_TYPE(__VA_ARGS__)> ID
...
#define BEGIN_SPECIALIZATION(ID, RESULT, ...)                       \
  ...
  virtual void* _yomm11_install() { return &::yorel::methods::register_spec<decltype(ID), ID ## _specialization>::the; } \
